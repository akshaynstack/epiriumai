...
class GPT3ComCon(discord.Cog, name="GPT3ComCon"):
    ...
    async def get_user_api_key(user_id, ctx):
        ...
        if user_api_key is None or user_api_key == "":
            ...
            if isinstance(ctx, discord.ApplicationContext):
                ...
                await ctx.reply(
                    "You must set up your API key before typing in a Epirium AI powered channel, type `/setup` to enter your API key."
                )
        return user_api_key
    ...
    async def end_conversation(
        ...
        if isinstance(ctx, discord.ApplicationContext):
            await ctx.respond(
                "You have ended the conversation with Epirium AI. Start a conversation with /gpt converse",
                ephemeral=True,
                delete_after=10,
            )
    ...
    async def send_usage_text(self, ctx):
        embed = discord.Embed(
            title="Epirium AI Usage", description="The current usage", color=0x00FF00
        )
    ...
    async def check_conversation_limit(self, message):
        ...
        if message.channel.id in self.conversation_threads:
            ...
            if (
                ...
                await message.reply(
                    "You have reached the maximum conversation length. You have ended the conversation with Epirium AI, and it has ended."
                )
            ...
            if message.channel.id in self.conversation_threads:
                ...
                if message.author.id in self.awaiting_responses:
                    message = await message.reply(
                        "You are already waiting for a response from Epirium AI. Please wait for it to respond before sending another message."
                    )
                ...
                if message.channel.id in self.awaiting_thread_responses:
                    message = await message.reply(
                        "This thread is already waiting for a response from Epirium AI. Please wait for it to respond before sending another message."
                    )
    ...
    @discord.slash_command(
        name="ask",
        description="Ask Epirium AI something!",
        guild_ids=ALLOWED_GUILDS,
    )
    @discord.option(
        name="prompt", description="The prompt to send to Epirium AI", required=True
    )
    @discord.option(
        name="temperature",
    ...
    @discord.slash_command(
        name="converse",
        description="Have a conversation with Epirium AI",
        guild_ids=ALLOWED_GUILDS,
    )
    @discord.option(
        name="opener",
    ...
    async def converse(
        ...
        if private:
            await ctx.respond(user.name + "'s private conversation with Epirium AI")
            thread = await ctx.channel.create_thread(
                name=user.name + "'s private conversation with Epirium AI",
                auto_archive_duration=60,
            )
        elif not private:
            message_thread = await ctx.respond(user.name + "'s conversation with Epirium AI")
            ...
            thread = await message_thread_real.create_thread(
                name=user.name + "'s conversation with Epirium AI",
                auto_archive_duration=60,
            )
        ...
        await thread.send(
            f"<@{str(user_id_normalized)}> You are now conversing with Epirium AI. *Say hi to start!*\n"
            ...
            f"If you want Epirium AI to ignore your messages, start your messages with `~`\n\n"
    ...
    @discord.slash_command(
        name="end",
        description="End a conversation with Epirium AI",
        guild_ids=ALLOWED_GUILDS,
    )
    @discord.guild_only()
